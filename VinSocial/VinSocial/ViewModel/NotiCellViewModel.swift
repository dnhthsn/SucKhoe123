...
class NotiCellViewModel :ObservableObject{
    @Published var noti: Notifications
    
    init(_ noti: Notifications) {
        self.noti = noti
    }
    
    var nameUser: String{
        return noti.user_info?.fullname ?? ""
    }
    ...
    var getStyleOf: String{
        return noti.styleof ?? ""
    }
    
    var getLinkAvatarUser: String {
        if noti.user_info == nil {
            return ""
        }
        if noti.user_info?.avatar == nil {
            return ""
        }
        if noti.user_info!.avatar!.contains("https://"){
            return noti.user_info?.avatar ?? ""
        }
        return "https://ws.suckhoe123.vn/"+(noti.user_info?.avatar ?? "")
    }
